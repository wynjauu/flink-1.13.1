...
package org.apache.flink.rocketmq.source.reader.deserializer;
...
import org.apache.flink.rocketmq.source.reader.deserializer.RowDeserializationSchema.MetadataConverter;
...
public class RocketMQRowDeserializationSchema implements RocketMQDeserializationSchema<RowData> {
    ...
    protected final DeserializationSchema<RowData> valueDeserialization;
    ...
    public void deserialize(
            List<MessageExt> input,
            Collector<RowData> collector) throws IOException {
        //extractMessages(input);
        //deserializationSchema.deserialize(bytesMessages, collector);
        if (CollectionUtils.isEmpty(input)) {
            return;
        }
        for (MessageExt message : input) {
            byte[] messageBody = message.getBody();
            valueDeserialization.deserialize(messageBody, collector);
        }
    }
