...
public class RocketMQRowDataConverter implements Serializable {
    ...
    public Message convert(RowData row) {
        ...
        if (hasMetadata) {
            String messageKeys = readMetadata(row, RocketMQDynamicTableSink.WritableMetadata.KEYS);
            message.setKeys(messageKeys);
            message.setTags(readMetadata(row, RocketMQDynamicTableSink.WritableMetadata.TAGS));
        }
        return message;
    }

    @SuppressWarnings("unchecked")
    private <T> T readMetadata(RowData consumedRow, RocketMQDynamicTableSink.WritableMetadata metadata) {
